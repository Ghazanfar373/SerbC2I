    ...
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
        ...
        public FlightData()
        {
            ...
            */
            ...
            MainSerb.comPort.ParamListChanged += FlightData_ParentChanged;
            ...
            for (int f = 1; f < 30; f++)
            {
                ...
                if (Settings.Instance["quickView" + f] != null)
                {
                    ...
                    if (ctls.Length > 0)
                    {
                        ...
                        if (QV.Tag == null)
                            QV.Tag = desc;
                        QV.desc = MainSerb.comPort.MAV.cs.GetNameandUnit(desc);
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        if (ctls.Length > 0)
                        {
                            ...
                            if (QV.Tag == null)
                                QV.Tag = desc;
                            QV.desc = MainSerb.comPort.MAV.cs.GetNameandUnit(QV.Tag.ToString());
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        public void BUT_playlog_Click(object sender, EventArgs e)
        {
            ...
            {
                MainSerb.comPort.logreadmode = false;
                ZedGraphTimer.Stop();
                playingLog = false;
            }
            else
            {
                ...
                MainSerb.comPort.logreadmode = true;
                list1.Clear();
                list2.Clear();
                list3.Clear();
                list4.Clear();
                ...
                list13.Clear();
                list14.Clear();
                list15.Clear();//Clear instruction inside outdie
                list16.Clear();
                list17.Clear();
                list18.Clear();
                list19.Clear();
        ...
        public void CheckBatteryShow()
        {
            ...
            if (MainSerb.comPort.MAV.param.ContainsKey("BATT_MONITOR") &&
                (float)MainSerb.comPort.MAV.param["BATT_MONITOR"] != 0)
            {
                hud1.batteryon = true;
            }
            else
        ...
        public void LoadLogFile(string file)
        {
            if (file != "")
            {
                try
                {
                    ...
                    MainSerb.comPort.logreadmode = true;
                    MainSerb.comPort.logplaybackfile = new BinaryReader(File.OpenRead(file));
                    MainSerb.comPort.lastlogread = DateTime.MinValue;
                    LBL_logfn.Text = Path.GetFileName(file);
                    log.Info("Open logfile " + file);
                    MainSerb.comPort.getHeartBeat();
                    tracklog.Value = 0;
                    tracklog.Minimum = 0;
        ...
        public void updatePayloadTabVisible()
        {
            ...
            if (MainSerb.comPort.compidcurrent == 1)
            {
                foreach (var mav in MainSerb.comPort.MAVlist)
                {
                    if (mav.sysid == MainSerb.comPort.sysidcurrent &&
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            {
                if (MainSerb.comPort.logplaybackfile != null)
                {
        ...
        {
            setupPropertyInfo(ref cust.Item, name, MainSerb.comPort.MAV.cs);
        ...
        private void BUT_clear_track_Click(object sender, EventArgs e)
        {
            ...
            if (MainSerb.comPort.MAV.camerapoints != null)
                MainSerb.comPort.MAV.camerapoints.Clear();
        }

        void but_Click(object sender, EventArgs e)
        {
            foreach (MainSwitcher.Screen sc in MainSerb.View.screens)
            {
                if (sc.Name == "FlightPlanner")
                {
                    splitContainerMap.Panel2.Controls.Remove(sc.Control);
                    splitContainerMap.Panel2.Controls.Remove((Control)sender);
            ...
            foreach (Control ctl in splitContainerMap.Panel2.Controls)
            {
                ctl.Visible = true;
            }
        }
        ...
        {
            if (MainSerb.comPort.MAV.cs.altoffsethome != 0)
            {
                MainSerb.comPort.MAV.cs.altoffsethome = 0;
            }
            else
            {
                MainSerb.comPort.MAV.cs.altoffsethome =
                    (float)(-MainSerb.comPort.MAV.cs.HomeAlt / CurrentState.multiplieralt);
            }
        }
        ...
        private void BUT_SendMSG_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (DialogResult.Cancel == InputBox.Show("Enter Message", "Enter Message to be logged", ref txt))
                    return;
                MainSerb.comPort.send_text(5, txt);
            }
            catch
            {
                CustomMessageBox.Show(Strings.ErrorNoResponce, Strings.ERROR);
        ...
        private void BUT_loadtelem_Click(object sender, EventArgs e)
        {
            ...
            if (MainSerb.comPort.logplaybackfile != null)
            {
                try
                {
                    MainSerb.comPort.logplaybackfile.Close();
                    MainSerb.comPort.logplaybackfile = null;
                }
                catch
                {
                }
        ...
        private void BUT_resetGimbalPos_Click(object sender, EventArgs e)
        {
            ...
            trackBarYaw.Value = 0;
            MainSerb.comPort.setMountConfigure(MAVLink.MAV_MOUNT_MODE.MAVLINK_TARGETING, false, false, false);
            MainSerb.comPort.setMountControl((float)trackBarPitch.Value * 100.0f, (float)trackBarRoll.Value * 100.0f,
        ...
        private void BUT_resumemis_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (MainSerb.comPort.BaseStream.IsOpen)
                {
                    string lastwp = MainSerb.comPort.MAV.cs.lastautowp.ToString();
                    if (lastwp == "-1")
                        lastwp = "1";
                    ...
                    if (InputBox.Show("Resume at", "Resume mission at waypoint#", ref lastwp) == DialogResult.OK)
                    {
                        ...
                        // get our target wp
                        var lastwpdata = MainSerb.comPort.getWP((ushort)lastwpno);

                        // get all
                        List<Locationwp> cmds = new List<Locationwp>();

                        var wpcount = MainSerb.comPort.getWPCount();

                        for (ushort a = 0; a < wpcount; a++)
                        {
                            var wpdata = MainSerb.comPort.getWP(a);
                        ...
                        MainSerb.comPort.setWPTotal((ushort)(cmds.Count));

                        // add our do commands
                        foreach (var loc in cmds)
                        {
                            MAVLink.MAV_MISSION_RESULT ans = MainSerb.comPort.setWP(loc, wpno,
                        ...
                        // set index back to 1
                        MainSerb.comPort.setWPCurrent(MainSerb.comPort.MAV.sysid, MainSerb.comPort.MAV.compid, 1);

                        if (MainSerb.comPort.MAV.cs.firmware == Firmwares.ArduCopter2)
                        {
                            while (MainSerb.comPort.MAV.cs.mode.ToLower() != "Guided".ToLower())
                            {
                            ...
                            timeout = 0;
                            while (!MainSerb.comPort.MAV.cs.armed)
                            {
                                MainSerb.comPort.doARM(true);
                                Thread.Sleep(1000);
                                Application.DoEvents();
                                timeout++;
                            ...
                            timeout = 0;
                            while (MainSerb.comPort.MAV.cs.alt < (lastwpdata.alt - 2))
                            {
                                if (!MainSerb.comPort.doCommand((byte)MainSerb.comPort.sysidcurrent,
                                    (byte)MainSerb.comPort.compidcurrent, MAVLink.MAV_CMD.TAKEOFF, 0, 0, 0, 0, 0, 0,
                                    lastwpdata.alt))
                                {
                                    CustomMessageBox.Show(Strings.CommandFailed, Strings.ERROR);
                                    return;
                        ...
                        timeout = 0;
                        while (MainSerb.comPort.MAV.cs.mode.ToLower() != "AUTO".ToLower())
                        {
                            MainSerb.comPort.setMode("AUTO");
                            Thread.Sleep(1000);
                            Application.DoEvents();
        ...
        private void CB_tuning_CheckedChanged(object sender, EventArgs e)
        {
            if (CB_tuning.Checked)
            {
                splitContainerMap.Panel1Collapsed = false;
            ...
            {
                splitContainerMap.Panel1Collapsed = true;
        ...
        void chk_box_tunningCheckedChanged(object sender, EventArgs e)
        {
            ...
            if (((CheckBox)sender).Checked)
            {
                ...
                else if (list13item == null)
                {
                    ...
                    }  //Ronday Badal meenh barsaindy aja sohniya dooron dooron kendy
                }
        ...
        void chk_box_quickview_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (checkbox.Checked)
            {
                ...
                desc = MainSerb.comPort.MAV.cs.GetNameandUnit(desc);
        ...
        private void FlightData_ParentChanged(object sender, EventArgs e)
        {
            ...
            {
                MainSerb.cam.camimage += cam_camimage;
            }

            // QUAD
            if (MainSerb.comPort.MAV.param.ContainsKey("WP_SPEED_MAX"))
            {
                try
                {
                    modifyandSetSpeed.Value = (decimal)((float)MainSerb.comPort.MAV.param["WP_SPEED_MAX"] / 100.0);
                }
                catch
                {
            ...
            // plane 3.7 and below with airspeed, uses ARSPD_ENABLE:
            else if ((MainSerb.comPort.MAV.param.ContainsKey("TRIM_ARSPD_CM") &&
                      MainSerb.comPort.MAV.param.ContainsKey("ARSPD_ENABLE")
                      && MainSerb.comPort.MAV.param.ContainsKey("ARSPD_USE") &&
                      (float)MainSerb.comPort.MAV.param["ARSPD_ENABLE"] == 1
                      && (float)MainSerb.comPort.MAV.param["ARSPD_USE"] == 1) ||
                     ...
                     (MainSerb.comPort.MAV.param.ContainsKey("TRIM_ARSPD_CM") &&
                      MainSerb.comPort.MAV.param.ContainsKey("ARSPD_TYPE")
                      && MainSerb.comPort.MAV.param.ContainsKey("ARSPD_USE") &&
                      (float)MainSerb.comPort.MAV.param["ARSPD_TYPE"] > 0
                      && (float)MainSerb.comPort.MAV.param["ARSPD_USE"] == 1))
            {
                try
                {
                    modifyandSetSpeed.Value = (decimal)((float)MainSerb.comPort.MAV.param["TRIM_ARSPD_CM"] / 100.0);
                }
                catch
                {
                    modifyandSetSpeed.Enabled = false;
            ...
            else if (MainSerb.comPort.MAV.param.ContainsKey("TRIM_THROTTLE") &&
                     MainSerb.comPort.MAV.param.ContainsKey("ARSPD_USE")
                     && (float)MainSerb.comPort.MAV.param["ARSPD_USE"] == 0)
            {
                try
                {
                    modifyandSetSpeed.Value = (decimal)(float)MainSerb.comPort.MAV.param["TRIM_THROTTLE"];
                }
                catch
                {
                    modifyandSetSpeed.Enabled = false;
            ...
            try
            {
                if (MainSerb.comPort.MAV.param.ContainsKey("LOITER_RAD"))
                    modifyandSetLoiterRad.Value =
                        (decimal)((float)MainSerb.comPort.MAV.param["LOITER_RAD"] * CurrentState.multiplierdist);
            }
            catch
            {
            ...
            try
            {
                if (MainSerb.comPort.MAV.param.ContainsKey("WP_LOITER_RAD"))
                {
                    modifyandSetLoiterRad.Value =
                        (decimal)((float)MainSerb.comPort.MAV.param["WP_LOITER_RAD"] * CurrentState.multiplierdist);
                }
            }
            catch
            {
        ...
        private void flightPlannerToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            foreach (MainSwitcher.Screen sc in MainSerb.View.screens)
            {
                if (sc.Name == "FlightPlanner")
                {
                    ...
                    {
                        Location = new Point(splitContainerMap.Panel2.Width / 2, 0),
                        Text = "Close"
                    };
                    but.Click += but_Click;

                    splitContainerMap.Panel2.Controls.Add(but);
                    splitContainerMap.Panel2.Controls.Add(sc.Control);
        ...
        private void groundColorToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                // green
                hud1.groundColor1 = Color.FromArgb(0xad, 0x68, 0x0D);
                hud1.groundColor2 = Color.FromArgb(0xf7, 0x93, 0x11);
        ...
        private void mainloop()
        {
            ...
            while (threadrun)
            {
                ...
                // log playback
                if (MainSerb.comPort.logreadmode && MainSerb.comPort.logplaybackfile != null)
                {
                    if (MainSerb.comPort.BaseStream.IsOpen)
                    {
                        MainSerb.comPort.logreadmode = false;
                    ...
                    DateTime logplayback = MainSerb.comPort.lastlogread;
                    try
                    {
                        if (!MainSerb.comPort.giveComport)
                        {
                            //await MainSerb.comPort.readPacketAsync().ConfigureAwait(false);
                            MainSerb.comPort.readPacketAsync().AwaitSync();
                        }
                    }
                    catch
                    {
                    ...
                    double act = (MainSerb.comPort.lastlogread - logplayback).TotalMilliseconds;
                    ...
                    if (LogPlayBackSpeed >= 4 && MainSerb.speechEnable)
                    ...
                    if (timetook != 0)
                    {
                        ...
                        //Console.WriteLine(MainSerb.comPort.lastlogread.Second + " " + DateTime.Now.Second + " " + (MainSerb.comPort.lastlogread.Second - DateTime.Now.Second));
                    ...
                    foreach (var MAV in MainSerb.comPort.MAVlist)
                    {
                        try
                        {
                            MAV.cs.UpdateCurrentSettings(null, false, MainSerb.comPort, MAV);
                        }
                        catch (Exception ex)
                        {
                            log.Error(ex);
                    ...
                    {
                        if (MainSerb.comPort.logplaybackfile != null &&
                            MainSerb.comPort.logplaybackfile.BaseStream.Position ==
                            MainSerb.comPort.logplaybackfile.BaseStream.Length)
                        {
                            MainSerb.comPort.logreadmode = false;
                        }
                    }
                ...
                else
                {
                    ...
                    if (!playingLog && MainSerb.comPort.logplaybackfile != null)
                    {
                        continue;
                    }
                }
                try
                {
                       ...
                       tracklast.AddSeconds(2) < DateTime.Now)
                    {
                        ...
                        lock (MainSerb.instance.adsblock)
                        {
                            // draw OA_DB
                            foreach (adsb.PointLatLngAltHdg plla in MainSerb.instance.adsbPlanes.Values)
                            {
                                if (plla.Raw != null)
                                {
                            ...
                            var adsbitems = MainSerb.instance.adsbPlanes.Values.Where(a =>
                            {
                                if (a.Raw != null)
                                {
        ...
        {
            var messagetime = MainSerb.comPort.MAV.cs.messages.LastOrDefault().time;
            if (messagecount != messagetime.toUnixTime())
            {
                try
                {
                    StringBuilder message = new StringBuilder();
                    MainSerb.comPort.MAV.cs.messages.ForEach(x =>
                    {
        ...
        private void modifyandSetAlt_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                MainSerb.comPort.setNewWPAlt(new Locationwp { alt = newalt / CurrentState.multiplieralt });
            }
            catch
            {
        ...
        private void PointCameraCoordsToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            ...
            if (split.Length == 3)
            {
                ...
                MainSerb.comPort.doCommandInt((byte)MainSerb.comPort.sysidcurrent, (byte)MainSerb.comPort.compidcurrent,
        ...
        {
            if (MainSerb.DisplayConfiguration.lockQuickView)
                return;
        ...
        private void setEKFHomeHereToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            MAVLink.mavlink_set_gps_global_origin_t go = new MAVLink.mavlink_set_gps_global_origin_t()
            {
                ...
                target_system = MainSerb.comPort.MAV.sysid
            };

            MainSerb.comPort.sendPacket(go, MainSerb.comPort.MAV.sysid, MainSerb.comPort.MAV.compid);
        }
        ...
        private async void setHomeHereToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (MainSerb.comPort.BaseStream.IsOpen)
            {
                try
                {
                    ...
                    {
                        MainSerb.comPort.doCommand((byte)MainSerb.comPort.sysidcurrent,
        ...
        {
            BeginInvoke((Action)delegate { gMapControl1.Bearing = (int)((MainSerb.comPort.MAV.cs.yaw + 360) % 360); });
        }
        ...
        private void startCameraToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (MainSerb.cam == null)
            {
                try
                {
                    MainSerb.cam = new Capture(Settings.Instance.GetInt32("video_device"), new AMMediaType());

                    MainSerb.cam.Start();

                    MainSerb.cam.camimage += new CamImage(cam_camimage);
                }
                catch (Exception ex)
                {
        ...
        private void tracklog_Scroll(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MainSerb.comPort.logplaybackfile != null)
                    MainSerb.comPort.logplaybackfile.BaseStream.Position =
                        (long)(MainSerb.comPort.logplaybackfile.BaseStream.Length * (tracklog.Value / 100.0));
            ...
            BeginInvoke((Action)delegate
            {
                try
                {
                    if (updatetracklog && tracklog.Visible)
                    {
                        ...
                        tracklog.Value = (int)(MainSerb.comPort.logplaybackfile.BaseStream.Position /
                            (double)MainSerb.comPort.logplaybackfile.BaseStream.Length * 100);
                        tracklog.ValueChanged += tracklog_Scroll;
                    }
                    ...
                    if (lbl_logpercent.Visible)
                        lbl_logpercent.Text =
                            (MainSerb.comPort.logplaybackfile.BaseStream.Position /
                             (double)MainSerb.comPort.logplaybackfile.BaseStream.Length).ToString("0.00%");
        ...
        private void tabStatus_Paint(object sender, PaintEventArgs e)
        {
            ...
            var list = MainSerb.comPort.MAV.cs.GetItemList(true);
        ...
        private void IDENT_btn_Click(object sender, EventArgs e)
        {
            ...
            MainSerb.comPort.uAvionixADSBControl(int.MaxValue,
        ...
        private void XPDRConnect_btn_Click(object sender, EventArgs e)
        {
            try
            {
                MainSerb.comPort.doCommand(MAVLink.MAV_CMD.SET_MESSAGE_INTERVAL, (float)MAVLink.MAVLINK_MSG_ID.UAVIONIX_ADSB_OUT_STATUS, (float)1000000.0, 0, 0, 0, 0, 0);
                var start = DateTime.Now;
                while (!MainSerb.comPort.MAV.cs.xpdr_status_pending && (DateTime.Now - start).TotalSeconds < 3) ; // wait until we receive a status message
                if (MainSerb.comPort.MAV.cs.xpdr_status_pending)
                {
                    updateTransponder();
                }
        ...
        {
            MainSerb.comPort.MAV.cs.xpdr_status_pending = false;
            if (!MainSerb.comPort.MAV.cs.xpdr_status_unavail)
            {
                ...
                {
                    Mode_clb.SetItemChecked(0, MainSerb.comPort.MAV.cs.xpdr_mode_A_enabled);
                    Mode_clb.SetItemChecked(1, MainSerb.comPort.MAV.cs.xpdr_mode_C_enabled);
                    Mode_clb.SetItemChecked(2, MainSerb.comPort.MAV.cs.xpdr_mode_S_enabled);
                    Mode_clb.SetItemChecked(3, MainSerb.comPort.MAV.cs.xpdr_es1090_tx_enabled);
                ...
                fault_clb.SetItemChecked(0, MainSerb.comPort.MAV.cs.xpdr_maint_req);
                fault_clb.SetItemChecked(1, MainSerb.comPort.MAV.cs.xpdr_gps_unavail);
                fault_clb.SetItemChecked(2, MainSerb.comPort.MAV.cs.xpdr_gps_no_fix);
                fault_clb.SetItemChecked(3, MainSerb.comPort.MAV.cs.xpdr_adsb_tx_sys_fail);
                fault_clb.SetItemChecked(4, MainSerb.comPort.MAV.cs.xpdr_airborne_status);

                if (!FlightID_tb.Focused)
                {
                    FlightID_tb.TextChanged -= new EventHandler(FlightID_tb_TextChanged);
                    FlightID_tb.Text = System.Text.Encoding.UTF8.GetString(MainSerb.comPort.MAV.cs.xpdr_flight_id);
                    FlightID_tb.TextChanged += new EventHandler(FlightID_tb_TextChanged);
                }
                ...
                {
                    Squawk_nud.ValueChanged -= new EventHandler(Squawk_nud_ValueChanged);
                    Squawk_nud.Value = (decimal)MainSerb.comPort.MAV.cs.xpdr_mode_A_squawk_code;
                    Squawk_nud.ValueChanged += new EventHandler(Squawk_nud_ValueChanged);
                }

                NIC_tb.Text = NIC_table[MainSerb.comPort.MAV.cs.xpdr_nic];
                NACp_tb.Text = NACp_table[MainSerb.comPort.MAV.cs.xpdr_nacp];

                IDENT_btn.Font = new Font(IDENT_btn.Font, MainSerb.comPort.MAV.cs.xpdr_ident_active ? FontStyle.Bold : FontStyle.Regular);
        ...
        private void gMapControl1_Scroll(object sender, ScrollEventArgs e)
        {
