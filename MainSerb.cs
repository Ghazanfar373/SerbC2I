...
{
    public partial class MainSerb : Form
    {
        ...
        public static bool speechEnabled()
        {
            ...
            {
                return MainSerb.comPort.MAV.cs.armed;
            }
            return true;
        }
        ...
        /// </summary>
        public static MainSerb instance = null;
        ...
        public  GCSViews.FlightStatus flightStatus;
        public GCSViews.MainBoard mainBoard;
    ...
    new ConcurrentDictionary<string, adsb.PointLatLngAltHdg>();
        ...
        public MainSerb()
        {
            ...
            //    lock (adsblock)
            //    {
            //        if (MainSerb.instance.adsbPlanes.ContainsKey(tuple.id))
            //        {
            //            // update existing
            ...
            //   Warnings.WarningEngine.WarningMessage += (sender, s) => { MainSerb.comPort.MAV.cs.messageHigh = s; };
            ...
            {
                MainSerb.ShowAirports = bool.Parse(Settings.Instance["showairports"]);
            }
            ...
            {
                MainSerb.ShowTFR = Settings.Instance.GetBoolean("showtfr", ShowTFR);
            }

            if (Settings.Instance["enableadsb"] != null)
            {
                MainSerb.instance.EnableADSB = Settings.Instance.GetBoolean("enableadsb");
            }
            ...
            try
            {
                ...
                //FlightStatus = new GCSViews.FlightStatus();
                flightStatus = new GCSViews.FlightStatus();
                log.Info("Create FP");
                FlightPlanner = new GCSViews.FlightPlanner();
                
                mainBoard = new GCSViews.MainBoard();
            ...
            if (Settings.Instance["CHK_GDIPlus"] != null)
            {
                ...
                GCSViews.FlightStatus.myhud.opengl = !bool.Parse(Settings.Instance["CHK_GDIPlus"].ToString());
            }

            if (Settings.Instance["CHK_hudshow"] != null)
            {
                //GCSViews.FlightData.myhud.hudon = bool.Parse(Settings.Instance["CHK_hudshow"].ToString());
                GCSViews.FlightStatus.myhud.hudon = !bool.Parse(Settings.Instance["CHK_hudshow"].ToString());
            }
            ...
            try
            {
                ...
                MainSerb.comPort.MAV.cs.ResetInternals();

                if (Settings.Instance["speechenable"] != null)
                    //  MainSerb.speechEnable = Settings.Instance.GetBoolean("speechenable");
                ...
                try
                {
                    if (Settings.Instance["TXT_homelat"] != null)
                        MainSerb.comPort.MAV.cs.PlannedHomeLocation.Lat = Settings.Instance.GetDouble("TXT_homelat");

                    if (Settings.Instance["TXT_homelng"] != null)
                        MainSerb.comPort.MAV.cs.PlannedHomeLocation.Lng = Settings.Instance.GetDouble("TXT_homelng");

                    if (Settings.Instance["TXT_homealt"] != null)
                        MainSerb.comPort.MAV.cs.PlannedHomeLocation.Alt = Settings.Instance.GetDouble("TXT_homealt");

                    // remove invalid entrys
                    if (Math.Abs(MainSerb.comPort.MAV.cs.PlannedHomeLocation.Lat) > 90 ||
                        Math.Abs(MainSerb.comPort.MAV.cs.PlannedHomeLocation.Lng) > 180)
                        MainSerb.comPort.MAV.cs.PlannedHomeLocation = new PointLatLngAlt();
                }
                catch
                {
                }
        ...
        private bool joysendThreadExited;
        ...
        public void updateLayout(object sender, EventArgs e)
        {
            ...
            //Flight data page
            //if (MainSerb.instance.FlightData != null)
            //{
            //    TabControl t = MainSerb.instance.FlightData.tabControlactions;
            ...
            //    MainSerb.instance.FlightData.loadTabControlActions();
            //}

            if (MainSerb.instance.FlightPlanner != null)
            {
                //hide menu items
                MainSerb.instance.FlightPlanner.updateDisplayView();
            }
        }
        void adsb_UpdatePlanePosition(object sender, MissionPlanner.Utilities.adsb.PointLatLngAltHdg adsb)
        {
            lock (adsblock)
            {
                ...
                if (MainSerb.instance.adsbPlanes.ContainsKey(id))
                {
                    // update existing
                ...
                {
                    // create new plane
                    MainSerb.instance.adsbPlanes[id] =
                ...
                try
                {
                    ...
                    packet.altitude = (int)(MainSerb.instance.adsbPlanes[id].Alt * 1000);
                    ...
                    packet.heading = (ushort)(MainSerb.instance.adsbPlanes[id].Heading * 100);
                    packet.lat = (int)(MainSerb.instance.adsbPlanes[id].Lat * 1e7);
                    packet.lon = (int)(MainSerb.instance.adsbPlanes[id].Lng * 1e7);
                    ...
                    MainSerb.comPort.sendPacket(packet, MainSerb.comPort.MAV.sysid, MainSerb.comPort.MAV.compid);
                }
                catch
                {
        ...
        {
            log.Info($"Mav Changed {MainSerb.comPort.MAV.sysid}");
        ...
        private void SaveConfig()
        {
            try
            {
                ...
                Settings.Instance.APMFirmware = MainSerb.comPort.MAV.cs.firmware.ToString();
                    ...
                    if (!MONO)
                    {
                        ...
                        if (joystick != null && joystick.enabled)
                        {
                            if (!joystick.manual_control)
                            {
                                ...
                                if (joystick.getJoystickAxis(1) != Joystick.joystickaxis.None)
                                    rc.chan1_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech1;
                                if (joystick.getJoystickAxis(2) != Joystick.joystickaxis.None)
                                    rc.chan2_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech2;
                                if (joystick.getJoystickAxis(3) != Joystick.joystickaxis.None)
                                    rc.chan3_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech3;
                                if (joystick.getJoystickAxis(4) != Joystick.joystickaxis.None)
                                    rc.chan4_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech4;
                                if (joystick.getJoystickAxis(5) != Joystick.joystickaxis.None)
                                    rc.chan5_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech5;
                                if (joystick.getJoystickAxis(6) != Joystick.joystickaxis.None)
                                    rc.chan6_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech6;
                                if (joystick.getJoystickAxis(7) != Joystick.joystickaxis.None)
                                    rc.chan7_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech7;
                                if (joystick.getJoystickAxis(8) != Joystick.joystickaxis.None)
                                    rc.chan8_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech8;
                                if (joystick.getJoystickAxis(9) != Joystick.joystickaxis.None)
                                    rc.chan9_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech9;
                                if (joystick.getJoystickAxis(10) != Joystick.joystickaxis.None)
                                    rc.chan10_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech10;
                                if (joystick.getJoystickAxis(11) != Joystick.joystickaxis.None)
                                    rc.chan11_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech11;
                                if (joystick.getJoystickAxis(12) != Joystick.joystickaxis.None)
                                    rc.chan12_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech12;
                                if (joystick.getJoystickAxis(13) != Joystick.joystickaxis.None)
                                    rc.chan13_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech13;
                                if (joystick.getJoystickAxis(14) != Joystick.joystickaxis.None)
                                    rc.chan14_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech14;
                                if (joystick.getJoystickAxis(15) != Joystick.joystickaxis.None)
                                    rc.chan15_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech15;
                                if (joystick.getJoystickAxis(16) != Joystick.joystickaxis.None)
                                    rc.chan16_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech16;
                                if (joystick.getJoystickAxis(17) != Joystick.joystickaxis.None)
                                    rc.chan17_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech17;
                                if (joystick.getJoystickAxis(18) != Joystick.joystickaxis.None)
                                    rc.chan18_raw = (ushort)MainSerb.comPort.MAV.cs.rcoverridech18;

                                if (lastjoystick.AddMilliseconds(rate) < DateTime.Now)
                                {
                                    /*
                                if (MainSerb.comPort.MAV.cs.rssi > 0 && MainSerb.comPort.MAV.cs.remrssi > 0)
                                {
                                    if (lastratechange.Second != DateTime.Now.Second)
                                    {
                                        if (MainSerb.comPort.MAV.cs.txbuffer > 90)
                                        {
                                            if (rate < 20)
                                                rate = 21;
                                            rate--;

                                            if (MainSerb.comPort.MAV.cs.linkqualitygcs < 70)
                                                rate = 50;
                                        }
                                        else
                                        {
                            ...
                            else
                            {
                                ...
                                if (joystick.getJoystickAxis(1) != Joystick.joystickaxis.None)
                                    rc.x = MainSerb.comPort.MAV.cs.rcoverridech1;
                                if (joystick.getJoystickAxis(2) != Joystick.joystickaxis.None)
                                    rc.y = MainSerb.comPort.MAV.cs.rcoverridech2;
                                if (joystick.getJoystickAxis(3) != Joystick.joystickaxis.None)
                                    rc.z = MainSerb.comPort.MAV.cs.rcoverridech3;
                                if (joystick.getJoystickAxis(4) != Joystick.joystickaxis.None)
                                    rc.r = MainSerb.comPort.MAV.cs.rcoverridech4;
            ...
            while (serialThread)
            {
                try
                {
                    ...
                    if (speechEnabled() && speechEngine != null && (DateTime.Now - speechcustomtime).TotalSeconds > 30 &&
                        (MainSerb.comPort.logreadmode || comPort.BaseStream.IsOpen))
                    {
                        if (MainSerb.speechEngine.IsReady)
                        {
                            if (Settings.Instance.GetBoolean("speechcustomenabled"))
                            {
                                MainSerb.speechEngine.SpeakAsync(ArduPilot.Common.speechConversion(comPort.MAV,
                        ...
                        if (Settings.Instance.GetBoolean("speechbatteryenabled") == true &&
                            MainSerb.comPort.MAV.cs.battery_voltage <= warnvolt &&
                            MainSerb.comPort.MAV.cs.battery_voltage >= 5.0)
                        {
                            if (MainSerb.speechEngine.IsReady)
                            {
                                MainSerb.speechEngine.SpeakAsync(ArduPilot.Common.speechConversion(comPort.MAV,
                                 ...
                                 (MainSerb.comPort.MAV.cs.battery_remaining) < warnpercent &&
                                 MainSerb.comPort.MAV.cs.battery_voltage >= 5.0 &&
                                 MainSerb.comPort.MAV.cs.battery_remaining != 0.0)
                        {
                            if (MainSerb.speechEngine.IsReady)
                            {
                                MainSerb.speechEngine.SpeakAsync(
                        ...
                        (MainSerb.comPort.logreadmode || comPort.BaseStream.IsOpen))
                    {
                            ...
                            MainSerb.comPort.MAV.cs.armed)
                        {
                            ...
                            if (MainSerb.comPort.MAV.cs.airspeed < warnairspeed)
                            {
                                if (MainSerb.speechEngine.IsReady)
                                {
                                    MainSerb.speechEngine.SpeakAsync(
                            ...
                            else if (MainSerb.comPort.MAV.cs.groundspeed < warngroundspeed)
                            {
                                if (MainSerb.speechEngine.IsReady)
                                {
                                    MainSerb.speechEngine.SpeakAsync(
                        ...
                        (MainSerb.comPort.logreadmode || comPort.BaseStream.IsOpen))
                    {
                        ...
                        try
                        {
                            altwarningmax = (int)Math.Max(MainSerb.comPort.MAV.cs.alt, altwarningmax);

                            if (Settings.Instance.GetBoolean("speechaltenabled") == true &&
                                MainSerb.comPort.MAV.cs.alt != 0.00 &&
                                (MainSerb.comPort.MAV.cs.alt <= warnalt) && MainSerb.comPort.MAV.cs.armed)
                            {
                        ...
                        try
                        {
                            ...
                            if (MainSerb.speechEngine.IsReady &&
                                lastmessagehigh != MainSerb.comPort.MAV.cs.messageHigh &&
                                MainSerb.comPort.MAV.cs.messageHigh != null)
                            {
                                if (!MainSerb.comPort.MAV.cs.messageHigh.StartsWith("PX4v2 "))
                                {
                                    MainSerb.speechEngine.SpeakAsync(MainSerb.comPort.MAV.cs.messageHigh);
                                    lastmessagehigh = MainSerb.comPort.MAV.cs.messageHigh;
                                }
                            }
                        }
                    ...
                    // not doing anything
                    if (!MainSerb.comPort.logreadmode && !comPort.BaseStream.IsOpen)
                    {
                        altwarningmax = 0;
                    }

                    // attenuate the link qualty over time
                    if ((DateTime.Now - MainSerb.comPort.MAV.lastvalidpacket).TotalSeconds >= 1)
                    {
                        if (linkqualitytime.Second != DateTime.Now.Second)
                        {
                            MainSerb.comPort.MAV.cs.linkqualitygcs =
                                (ushort)(MainSerb.comPort.MAV.cs.linkqualitygcs * 0.8f);
                            ...
                            this.BeginInvokeIfRequired(
                                ...
                                delegate { //GCSViews.FlightData.myhud.Invalidate();
                                    GCSViews.FlightStatus.myhud.Invalidate();
                                });
                    ...
                    if ((DateTime.Now - MainSerb.comPort.MAV.lastvalidpacket).TotalSeconds > 3
                        && (DateTime.Now - connecttime).TotalSeconds > 30
                        && (DateTime.Now - nodatawarning).TotalSeconds > 5
                        && (MainSerb.comPort.logreadmode || comPort.BaseStream.IsOpen)
                        && MainSerb.comPort.MAV.cs.armed)
                    {
                        var msg = "WARNING No Data for " + (int)(DateTime.Now - MainSerb.comPort.MAV.lastvalidpacket).TotalSeconds + " Seconds";
                        MainSerb.comPort.MAV.cs.messageHigh = msg;
                        if (speechEnabled() && speechEngine != null)
                        {
                            if (MainSerb.speechEngine.IsReady)
                            {
                                MainSerb.speechEngine.SpeakAsync(msg);
                    ...
                    if (armedstatus != MainSerb.comPort.MAV.cs.armed && comPort.BaseStream.IsOpen)
                    {
                        armedstatus = MainSerb.comPort.MAV.cs.armed;
                        // status just changed to armed
                        if (MainSerb.comPort.MAV.cs.armed == true &&
                            MainSerb.comPort.MAV.apname != MAVLink.MAV_AUTOPILOT.INVALID &&
                            MainSerb.comPort.MAV.aptype != MAVLink.MAV_TYPE.GIMBAL)
                        {
                            System.Threading.ThreadPool.QueueUserWorkItem(state =>
                            {
                                ...
                                try
                                {
                                    ...
                                    MainSerb.comPort.MAV.cs.HomeLocation = new PointLatLngAlt(MainSerb.comPort.getWP(0));
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            MyView.AddScreen(new MainSwitcher.Screen("MainBoard", mainBoard, true));
            MyView.AddScreen(new MainSwitcher.Screen("FlightStatus", flightStatus, true));
            ...
            {
                this.PerformLayout();
                flightStatusToolStripMenuItem_Click(this, e);
               // menustr(this, new ToolStripItemClickedEventArgs(planningToolStripMenuItem));
            }
            else
            {
                this.PerformLayout();
                log.Info("show FlightStatus");
                flightStatusToolStripMenuItem_Click(this, e);
                log.Info("show FlightStatus... Done");
               // MainMenu_ItemClicked(this, new ToolStripItemClickedEventArgs(flightDataToolStripMenuItem));
            }
                ...
                try
                {
                    ...
                    MainSerb.instance.BeginInvoke((Action)delegate
                    {
                        if (MainSerb.comPort.BaseStream.IsOpen)
                        {
                            ...
                            MainSerb.instance.doConnect(mav, "preset", serial.PortName);
                        ...
                        {
                            MainSerb.comPort.BaseStream = serial;
                            MainSerb.instance.doConnect(MainSerb.comPort, "preset", serial.PortName);
                        }
                    });
                }
                catch (Exception ex)
            ...
            GStreamer.onNewImage += (sender, image) =>
            {
                try
                {
                    if (image == null)
                    {
                        GCSViews.FlightStatus.myhud.bgimage = null;
                        return;
                    }

                    var old = GCSViews.FlightStatus.myhud.bgimage;
                    GCSViews.FlightStatus.myhud.bgimage = new Bitmap(image.Width, image.Height, 4 * image.Width,
            ...
            vlcrender.onNewImage += (sender, image) =>
            {
                try
                {
                    ...
                    var old = GCSViews.FlightStatus.myhud.bgimage;
                    GCSViews.FlightStatus.myhud.bgimage = new Bitmap(image.Width,
                        image.Height,
                        4 * image.Width,
                        PixelFormat.Format32bppPArgb,
                    ...
                    try
                    {
                        ...
                        lock (locker)
                        {
                                ...
                                (int)System.Windows.Forms.DialogResult.Yes)
                            {
                                ...
                                {
                                    MainSerb.instance.doConnect(mav, "preset", port.ToString());
            ...
            {
                if (MainSerb.instance.InvokeRequired)
                {
                    log.Info("CommsSerialScan.doConnect invoke");
                    MainSerb.instance.BeginInvoke(
                        (Action)delegate ()
                        {
                            MAVLinkInterface mav = new MAVLinkInterface();
                            mav.BaseStream = port;
                            MainSerb.instance.doConnect(mav, "preset", "0");
                            MainSerb.Comports.Add(mav);
            ...
            if (Program.args.Length > 0)
            {
                ...
                {
                  //  FlightData.LoadLogFile(Program.args[0]);     Dobeta...Inline integration
                ...
                if (cmds.ContainsKey("joy") && cmds.ContainsKey("type"))
                {
                    ...
                    {
                        MainSerb.comPort.MAV.cs.firmware = Firmwares.ArduSub;
                    }

                    var joy = JoystickBase.Create(() => MainSerb.comPort);

                    if (joy.start(cmds["joy"]))
                    {
                        MainSerb.joystick = joy;
                        MainSerb.joystick.enabled = true;
                    }
                    else
                    {
                ...
                if (cmds.ContainsKey("rtk"))
                {
                    ...
                    if (cmds["rtk"].ToLower().Contains("http"))
                    {
                        ...
                        Task.Run(() =>
                        {
                            ...
                            {
                                nt.Open(cmds["rtk"]);
                                nt.lat = MainSerb.comPort.MAV.cs.PlannedHomeLocation.Lat;
                                nt.lng = MainSerb.comPort.MAV.cs.PlannedHomeLocation.Lng;
                                nt.alt = MainSerb.comPort.MAV.cs.PlannedHomeLocation.Alt;
                ...
                if (cmds.ContainsKey("cam"))
                {
                    try
                    {
                        MainSerb.cam = new WebCamService.Capture(int.Parse(cmds["cam"]), null);
                ...
                if (cmds.ContainsKey("port") && cmds.ContainsKey("baud"))
                {
                    ...
                    doConnect(MainSerb.comPort, cmds["port"], cmds["baud"]);
                }
            }
            ...
            try
            {
                ...
                this.BeginInvokeIfRequired(() =>
                {
                    ...
                    if (Settings.Instance.GetBoolean("loadwpsonconnect") == true && showui)
                    {
                        ...
                        if (comPort.BaseStream.IsOpen)
                        {
                            planningToolStripMenuItem_Click(null, null);
                            FlightPlanner.BUT_read_Click(null, null);
                        }
                    }

                    // get any rallypoints
                    if (MainSerb.comPort.MAV.param.ContainsKey("RALLY_TOTAL") &&
                        int.Parse(MainSerb.comPort.MAV.param["RALLY_TOTAL"].ToString()) > 0 && showui)
                    {
                        try
                        {
                    ...
                    // get any fences
                    if (MainSerb.comPort.MAV.param.ContainsKey("FENCE_TOTAL") &&
                        int.Parse(MainSerb.comPort.MAV.param["FENCE_TOTAL"].ToString()) > 1 &&
                        MainSerb.comPort.MAV.param.ContainsKey("FENCE_ACTION") && showui)
                    {
                        try
                        {
        ...
        private void connectionToolStripMenuItem_Click(object sender, EventArgs e)
        {
           ...
           // MessageBox.Show("Do you want see ConnectionBox?");
        }
        ...
        }   //comPort destroyed..... instead of layamekdom
        ...
        private void MainSerb_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void simulationsToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
